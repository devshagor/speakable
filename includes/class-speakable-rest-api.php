<?php
/**
 * REST API endpoints for Speakable.
 *
...
/**
 * Class SPEAKABLE_REST_API
 *
 * Registers read-only REST API routes under the speakable/v1 namespace.
 ...
 * administrator via Speakable > Settings > API tab.
 *
 * @since 1.0.0
 */
class SPEAKABLE_REST_API {
	 ...
	 * @var string
	 */
	const REST_NAMESPACE = 'speakable/v1';
	...
	public function register_routes() {
		...
		// GET /wp-json/speakable/v1/speech/{id}.
			...
			array(
				...
				'args'                => array(
					'id' => array(
						'required'          => true,
						'type'              => 'integer',
						'description'       => __( 'Post ID.', 'speakable' ),
		...
		// GET /wp-json/speakable/v1/settings.
		...
		// GET /wp-json/speakable/v1/posts.
			...
			array(
				...
				'args'                => array(
					'post_type' => array(
						'default'           => 'post',
						'type'              => 'string',
						'description'       => __( 'Post type slug.', 'speakable' ),
					...
					'per_page'  => array(
						'default'           => 10,
						'type'              => 'integer',
						'description'       => __( 'Results per page (max 50).', 'speakable' ),
						'sanitize_callback' => 'absint',
						'validate_callback' => function ( $param ) {
					...
					'page'      => array(
						'default'           => 1,
						'type'              => 'integer',
						'description'       => __( 'Page number.', 'speakable' ),
						'sanitize_callback' => 'absint',
						'validate_callback' => function ( $param ) {
							return is_numeric( $param ) && (int) $param > 0;
						},
					...
					'search'    => array(
						'default'           => '',
						'type'              => 'string',
						'description'       => __( 'Search query.', 'speakable' ),
	...
	public function get_speech_data( $request ) {
		...
		if ( ! $post || 'publish' !== $post->post_status ) {
			return new WP_Error(
				'speakable_post_not_found',
				__( 'Post not found or not published.', 'speakable' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! in_array( $post->post_type, $enabled_types, true ) ) {
			return new WP_Error(
				'speakable_not_enabled',
				__( 'Text-to-speech is not enabled for this post type.', 'speakable' ),
				array( 'status' => 403 )
			);
		}
	...
	private function html_to_plain_text( $html ) {
		...
		$html = preg_replace( '/<div class="speakable-player"[^>]*>.*?<\/div>\s*<\/div>/s', '', $html );
